...
def auto_open_enabled() -> bool:
    ...
    return env_bool("V1046_AUTO_RUN_ON_OPEN", True)
...
(function(){
  ...
  function setStatus(s){
    ...
    if(s.last_started_at) bits.push('開始 ' + s.last_started_at);
    if(s.last_finished_at) bits.push('完成 ' + s.last_finished_at);
...
def landing_page() -> str:
    ...
    rows = "".join(
        f"<tr><td>{folder}</td><td><a href='{url_for('download_file', folder=folder, name=p.name)}'>{p.name}</a></td><td>{p.stat().st_size:,}</td><td>{datetime.fromtimestamp(p.stat().st_mtime).strftime('%Y-%m-%d %H:%M:%S')}</td></tr>"
