...
LOG_DIR = ROOT / "logs"
for p in [STATE_DIR, OUTPUT_DIR, LOG_DIR]:
...
def append_run_log(ss, run_id: str, demo: bool, status: str, error: str = "") -> None:
    ...
    if not rows:
        ...
        rows = [header] + rows[1:]
    rows.append([run_id, now_ts(), "DEMO" if demo else "REAL", status, (error or "")[:1000], "gs-sync-v1", "V104.6"])
...
def sync_before_run(health: List[str], demo: bool = False) -> Optional[Dict[str, object]]:
    if not sheets_enabled():
        ...
        return None
    run_id = f"{'DEMO' if demo else 'REAL'}-{datetime.now().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}"
    ss = _spreadsheet()
...
def sync_after_run(ctx: Optional[Dict[str, object]], health: List[str], demo: bool, status: str = "success", error: str = "") -> None:
    if not sheets_enabled():
        return
    run_id = (ctx or {}).get("run_id") or f"{'DEMO' if demo else 'REAL'}-{datetime.now().strftime('%Y%m%d-%H%M%S')}-noctx"
